...
#### 1.3 내 정보 수정 (F-03)
- **위치**: `MovieArchiveService.updateProfile()`, `UserDAO.update()`
- **구현 내용**
...
#### 1.4 회원 탈퇴 (F-04)
- **위치**: `AuthController.deactivate()`, `MovieArchiveService.deactivate()`, `UserDAO.deactivate()`
- **구현 내용**
...
#### 2.1 영화 목록 조회 (F-05)
- **위치**: `MovieController.getAllMovies()`, `MovieService.getAllMovies()`, `MovieDAO.findAll()`
- **구현 내용**
...
#### 2.2 영화 상세 보기 (F-06)
- **위치**: `MovieController.getMovieDetail()`, `MovieService.getMovieDetail()`, `MovieDAO.findById()`
- **구현 내용**
  - 영화 ID로 단건 조회
  - 평균 별점을 함께 계산하여 표시
  - 감독, 줄거리, 포스터 이미지 표시
...
#### 2.3 장르별 필터링 (F-07)
- **위치**: `MovieController.filterByGenre()`, `MovieService.filterByGenre()`, `MovieDAO.findByGenre()`
- **구현 내용**
...
#### 2.5 영화 제목 검색 (F-15)
- **위치**: `MovieController.searchByTitle()`, `MovieService.searchByTitle()`, `MovieDAO.searchByTitle()`
- **구현 내용**
...
#### 2.6 감독 검색 (F-16)
- **위치**: `MovieController.searchByDirector()`, `MovieService.searchByDirector()`, `MovieDAO.searchByDirector()`
- **구현 내용**
  - 감독 이름으로 LIKE 쿼리 검색
  - 동일 감독의 모든 영화 조회
  - 공백 검증 처리

#### 2.7 TOP 5 평점 높은 영화 조회 (추가)
- **위치**: `MovieController.getTopRatedMovies()`, `MovieService.getTopRatedMovies()`, `MovieDAO.getTopRatedMovies()`
- **구현 내용**
  - 평균 별점이 높은 영화 상위 5개 조회
  - 정렬 기능으로 사용자 선호 영화 파악

#### 2.8 영화 관리 (관리자) (F-17)
- **위치**: `MovieController.addMovie()`, `MovieController.updateMovie()`, `MovieController.deleteMovie()`, `MovieService.addMovie()`, `MovieService.updateMovie()`, `MovieService.deleteMovie()`, `MovieDAO.insert()`, `MovieDAO.update()`, `MovieDAO.delete()`
- **구현 내용**
...
#### 3.1 리뷰 작성 (F-09)
- **위치**: `ReviewController.addReview()`, `ReviewService.addReview()`, `ReviewDAO.insert()`
- **구현 내용**
  - JSlider로 별점(1~5) 선택
  - JTextArea로 리뷰 내용 작성
  - 로그인 여부 검증 후 DB 저장
...
#### 3.2 리뷰 수정 (F-10)
- **위치**: `ReviewController.updateReview()`, `ReviewService.updateReview()`, `ReviewDAO.update()`
- **구현 내용**
  - 리뷰 ID로 기존 내용 불러오기
  - 작성자 본인 검증 후 수정 허용
  - 별점과 내용을 모두 수정 가능
...
#### 3.3 리뷰 삭제 (F-11)
- **위치**: `ReviewController.deleteReview()`, `ReviewService.deleteReview()`, `ReviewDAO.delete()`
- **구현 내용**
  - 본인 리뷰인지 확인
  - 관리자는 모든 리뷰 삭제 가능
  - 삭제 확인 다이얼로그 제공
...
#### 3.4 리뷰 목록 조회 (F-12)
- **위치**: `ReviewController.getReviewsByMovie()`, `ReviewService.getReviewsByMovie()`, `ReviewDAO.findByMovieId()`
- **구현 내용**
...
#### 3.5 내 리뷰 모아보기 (F-13)
- **위치**: `ReviewController.getMyReviews()`, `ReviewService.getMyReviews()`, `ReviewDAO.findByUserId()`
- **구현 내용**
...
#### 4.1 회원 및 권한 관리 (F-18)
- **위치**: `UserDAO.findAll()`, `UserDAO.updateRole()`, `AdminPanel`
- **구현 내용**
...
| **View** | `LoginFrame`, `MainFrame` | 메인 UI 화면 |
| | `MovieListPanel`, `MovieDetailPanel` | 영화 관련 UI |
| | `ReviewDialog` | 리뷰 관리 UI |
| | `MyPagePanel`, `AdminPanel` | 마이페이지/관리자 UI |
...
- **권한 분리**: 관리자 권한으로 관리 기능 제한
- **작성자 검증**: 리뷰 수정/삭제 시 작성자 본인 확인
- **데이터 연쇄 처리**: 사용자 탈퇴 시 연관 리뷰도 함께 삭제
